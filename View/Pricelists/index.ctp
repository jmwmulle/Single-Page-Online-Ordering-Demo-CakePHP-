<div class="pricelists index">
	<h2><?php echo __('Pricelists'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<thead>
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('p1'); ?></th>
			<th><?php echo $this->Paginator->sort('p2'); ?></th>
			<th><?php echo $this->Paginator->sort('p3'); ?></th>
			<th><?php echo $this->Paginator->sort('p4'); ?></th>
			<th><?php echo $this->Paginator->sort('p5'); ?></th>
			<th><?php echo $this->Paginator->sort('p6'); ?></th>
			<th><?php echo $this->Paginator->sort('label'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	</thead>
	<tbody>
	<?php foreach ($pricelists as $pricelist): ?>
	<tr>
		<td><?php echo h($pricelist['Pricelist']['id']); ?>&nbsp;</td>
		<td><?php echo h($pricelist['Pricelist']['p1']); ?>&nbsp;</td>
		<td><?php echo h($pricelist['Pricelist']['p2']); ?>&nbsp;</td>
		<td><?php echo h($pricelist['Pricelist']['p3']); ?>&nbsp;</td>
		<td><?php echo h($pricelist['Pricelist']['p4']); ?>&nbsp;</td>
		<td><?php echo h($pricelist['Pricelist']['p5']); ?>&nbsp;</td>
		<td><?php echo h($pricelist['Pricelist']['p6']); ?>&nbsp;</td>
		<td><?php echo h($pricelist['Pricelist']['label']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $pricelist['Pricelist']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $pricelist['Pricelist']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $pricelist['Pricelist']['id']), array(), __('Are you sure you want to delete # %s?', $pricelist['Pricelist']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</tbody>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Pricelist'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Orbopts'), array('controller' => 'orbopts', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Orbopt'), array('controller' => 'orbopts', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Orbs'), array('controller' => 'orbs', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Orb'), array('controller' => 'orbs', 'action' => 'add')); ?> </li>
	</ul>
</div>
