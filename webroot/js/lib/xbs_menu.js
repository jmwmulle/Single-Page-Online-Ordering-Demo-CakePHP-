...
//				});
		...
		load_from_cart: function (orb_id) {
			...
			if (XBS.cart.find_configuration(orb_id)) {
...
//			XBS.menu.load_orb_to_stage(active_orb_id);
...
Orbcard.prototype = {
	app: undefined,
	exposed_face: undefined,
	animation_queue: { animating: false, queued: 0, start: 0},

	init: function(app) {
		this.app = app;
		this.exposed_face = C.FRONT_FACE;
	},
	 ...
	 * @param price_rank
	 */
	show_back_face: function (price_rank) {
		app.menu.update_orb_form();
		this.orbcard_load_configuration();
		this.exposed_face = C.BACK_FACE;
		var row_menu_hide_time = 0;
		$(as_class(FX.swap_width)).each(function () {
			row_menu_hide_time = 800;
			this.toggle_row_menu($($(this).children(".orb-card-button")[0]).attr('id'), C.HIDE)
		});
		setTimeout(function () {
			if (!price_rank) price_rank = 0;
	...
	/**
	 * Flips state of orbopt menu element to enable or disable activation/configuration.
	 *
	 * @param element
	 * @returns {boolean}
	 */
	toggle_orb_opt: function (element) {
		var debug_this = 1;
		if (debug_this > 0) pr([element], "XBS.menu.toggle_orb_opt(element)", 2);
	...
	/**
	 * Flips state of orbopt icon element.
	 *
	 * @param element
	...
	toggle_orb_opt_icon: function (element, trigger_update) {
		...
		// TODO: trigger should issue an event on the Orb or at least a route that targets the Orb
		if (trigger_update === true) $(XBS.routing).trigger(C.ROUTE_REQUEST, {request:"order_update/ui", trigger:this});
		return true;
	},

	// PROBABLY SLATED FOR REMOVAL ENTIRELY
	...
	// TODO probably should be a couple of discreet & simpler finctions
		...
		$(XSM.menu.sauce).each(function() {
			...
			switch (weights.sauce) {
				...
				case 0.5:
					...
					if (!right_side_active) $(left_side_element).addClass(FX.active);
					break;
				case 1:
	...
	/**
	 * Builds the miniature list of configured orbopts in bottom-right of orbcard.
	 *
     * @param orb
     * @private
	 */
	_build_configured_opts_reference: function( orb ) {
		$(XSM.menu.tiny_orb_opts_list_wrapper).html( orb.generate_orbcard_opt_reference() );
		return true;
	},

	/**
	 * Maps Orb object's orbopt configuration to current orbopt menu.
	 *
	 * @param orb
	 * @private
	 */
	_load_configured_opts_to_ui: function( orb ) {
		$(orb.orbopts).each(function () {
			if (this.selected) {
				XBS.menu.toggle_orb_opt(this.html_id, false);
				XBS.menu.toggle_orb_opt_icon(this.icon(), false);
			}
		});
	},

	/**
	 *  Maps Orb object to orbcard UI. Note: orbcard configuration form hitherto addressed in XBS.cart.configure()
	 *
	 *  @param orb
	 */
	load_orb_configuration: function ( orb ) {
		XBS.menu._build_configured_opts_reference(orb);
		XBS.menu._load_configured_opts_to_ui(orb);

		XBS.layout.activize($(XSM.menu.orb_size_button + "[data-price-rank=" + orb.price_rank + "]"));
		return true;
	},
	 ...
	 * @param orb_id
	 */
	load_orb_to_stage: function (orb_id, data) {
		if (!data) throw "ServerError: JSON from server was malformed.";
		// todo: fallback on ajax fail
		var orb_card_stage = $.parseHTML(data.orb_card_stage);
		var orbopts_list = $.parseHTML(data.orbopts.list);
		var optflag_header = data.optflag_header;
		var replace_time = 0;
		var portionable = data.orbopts.portionable;
